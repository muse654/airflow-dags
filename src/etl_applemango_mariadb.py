...
class AppleMangoMariaDBETL(ETLBase):
    """AppleMango MariaDB에서 Target ClickHouse로 데이터 이관 (PK 기준 중복 제거)"""
    
    def __init__(self, table_name, query=None, primary_key=None):
        super().__init__(f"AppleMango_MariaDB_{table_name}")
    ...
    def extract(self):
        """MariaDB에서 데이터 추출"""
        try:
            self.source_conn = DatabaseConnector.get_mariadb_connection('APPLEMANGO')
    ...
    def load(self, data):
        ...
        try:
            ...
            target_table = f"applemango_{self.table_name}"
            ...
            logger.info(f"📊 최종 테이블 건수: {total_count:,}건 (중복 제거 후)")
    ...
    def _create_table_if_not_exists(self, table_name, df):
        """ClickHouse 테이블 자동 생성 (ReplacingMergeTree - 복합 PK 지원)"""
        type_mapping = {
            'int64': 'Int64',
            'int32': 'Int32',
            'uint64': 'UInt64',
        ...
        for col_name, dtype in df.dtypes.items():
            ...
            # PK와 source 컬럼은 NOT NULL (리스트에 포함 확인)
        ...
        logger.info(f"테이블 생성 SQL (PK: {pk_str}):\n{create_table_sql}")
        
        self.target_client.command(create_table_sql)
        logger.info(f"✅ 테이블 `{table_name}` 생성 완료 (ReplacingMergeTree)")
